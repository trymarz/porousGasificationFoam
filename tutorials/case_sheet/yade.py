...
# One DEM sphere at each selected OpenFOAM cell centre.
# Matches blockMeshDict: [-0.02, 0.02]^3 with 21 cells per direction.
rSphere = 0.00075
...
for i in range(nCellX):
    x = foamMin[0] + (i + 0.5)*dx
    if x < -hx - tol or x > hx + tol:
        continue

    for j in range(nCellY):
        y = foamMin[1] + (j + 0.5)*dy
        if y < -hy - tol or y > hy + tol:
            continue

        for k in range(nCellZ):
            z = foamMin[2] + (k + 0.5)*dz
            if z < -hz - tol or z > hz + tol:
                continue

            sid = O.bodies.append(
                sphere(Vector3(x, y, z), rSphere, material='spheremat')
            )
            sphereIDs.append(sid)

numspheres = len(sphereIDs)
print("[GEOMETRY] one sphere per selected OpenFOAM cell:", numspheres)
