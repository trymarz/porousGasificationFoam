...
# first PyRunner invocation (before PgfCoupling has set real values).
# Once clamped, max(new_rad, CHAR_CORE_RADIUS) keeps them stuck.
# Setting lambdaDot=1.0 preserves the as-created radius until
# PgfCoupling updates it from the Ychar-driven lambdaDot field.
for b in O.bodies:
...
# ── foam coupling (PGF-native backend) ────────────────────────────
# PgfCoupling exposes the same Python API as FoamCoupling (SetOpenFoamSolver,
# setIdList, killMPI, comm) but exchanges only particle kinematics (YADE→OF)
# and a per-particle lambdaDot shrink factor (OF→YADE). There is no
# isGaussianInterp flag — the fluid never pushes back on the particles.
fluidCoupling = PgfCoupling()
...
# PgfCoupling, and applied here every 1 ms of sim time.  Once
...
# (rank 0) keeps a full copy of every body, but PgfCoupling delivers the
...
# (PgfCoupling::syncLambdaDotToMaster): once a YADE built with that fix is in
...
def _sync_lambdaDot_master_slave():
    ...
    # PgfCoupling; broadcast its {id: lambdaDot} map to the master.
    ld_map = None
    if rank == 1:
