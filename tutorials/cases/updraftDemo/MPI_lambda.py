...
writeInterval = 0.5   # seconds — Yade VTK write interval (matches OF writeInterval)
...
# Particle bed region, in OpenFOAM mesh coordinates (column is
# x,y in [0, 0.05], z in [0, 0.5]; bed packed into z in [0.05, 0.30]).
# Positions MUST be absolute OF coordinates — the coupling locates each
# particle in an OF cell by its world position.
margin = 1e-3
mn = Vector3(0.005, 0.005, 0.05 + margin)
mx = Vector3(0.045, 0.045, 0.30 - margin)

sp = pack.SpherePack()
sp.makeCloud(mn, mx, rMean=0.004, rRelFuzz=0, num=numspheres)
