...
import subprocess  # to read controlDict
...
NSTEPS = int(round(foamEndTime / O.dt))
...
# One DEM sphere per OpenFOAM cell in an L in the central y-z plane.
# Each arm spans the cell centres from about -0.01 m to +0.01 m.
rSphere = 0.00075

foamMin = Vector3(-0.02, -0.02, -0.02)
foamMax = Vector3( 0.02,  0.02,  0.02)

nCellX = 21
nCellY = 21
nCellZ = 21

dx = (foamMax[0] - foamMin[0]) / nCellX
dy = (foamMax[1] - foamMin[1]) / nCellY
dz = (foamMax[2] - foamMin[2]) / nCellZ
...
kMin = 5
kMax = 15
cellStride = 2 # 1 sphere in every cellStride number of cells

selectedCells = []

# Vertical arm in y at the lower-z corner, one sphere every other cell.
for j in range(jMin, jMax + 1, cellStride):
    selectedCells.append((iCenter, j, kMin))

# Horizontal arm in z at the lower-y corner; skip the shared corner.
for k in range(kMin + cellStride, kMax + 1, cellStride):
    selectedCells.append((iCenter, jMin, k))

for i, j, k in selectedCells:
    center = Vector3(
        foamMin[0] + (i + 0.5)*dx,
        foamMin[1] + (j + 0.5)*dy,
        foamMin[2] + (k + 0.5)*dz
    )
    sid = O.bodies.append(sphere(center, rSphere, material='spheremat'))
    sphereIDs.append(sid)
    sphereByCell[(i, j, k)] = sid

numspheres = len(sphereIDs)
print("[GEOMETRY] one sphere per L-shape OpenFOAM cell:", numspheres)
...
# Connect consecutive spheres along each L-shape arm.
for cell, sid in sphereByCell.items():
    i, j, k = cell
    ...
    for neighborCell in (
        ...
        if neighborCell not in sphereByCell:
            continue

        sid2 = sphereByCell[neighborCell]
        L0 = (O.bodies[sid2].state.pos - O.bodies[sid].state.pos).norm()
        springsList[spring_id_counter] = (sid, sid2, L0)
        spring_id_counter += 1
...
def printAndSaveDtInfo():
    ...
    if mp.rank == 0:
        ...
        with open("dtInfo.txt", "a") as f:
            if write_header:
                f.write("iter time yadeDt foamDt ratio NTSTP\n")
            f.write(f"{O.iter} {O.time:.2e} {yadeDt:.3e} {foamDt:.3e} {ratio:.1f} {NSTEPS:.1f}\n")
