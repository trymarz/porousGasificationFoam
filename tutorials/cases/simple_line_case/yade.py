...
young = 25e6 #5e6
density = 1200
NSTEPS = 240000000 #int(2e5)
#fixed_seed = 12345 # to keep random arrangment of spheres repertable. if changing this value, the arrangment will be different.
...
reference_radius = 0.00075

# Preserve the effective mass and inertia used in the original case.
effective_mass = (
    density
    * (4.0 / 3.0)
    * math.pi
...
# blockmeshdict mesh cell size for a 0.04 m box with 21 cells.

mesh_cell_size = 0.04 / 21.0

# The seven-cell line has end-cell centres at z = +/-3*cellSize.
line_end_cell_center = 4.0 * mesh_cell_size

particle_positions = [
    Vector3(0.0, 0.0, -line_end_cell_center),
    Vector3(0.0, 0.0,  line_end_cell_center)
]

# Create exactly two deterministic particles.
O.bodies.append(
    [
        sphere
        (
...
# Keep the dynamics of the old large particles (eference_radius)
for b in O.bodies:
    if isinstance(b.shape, Sphere):
        b.state.mass = effective_mass
        b.state.inertia = Vector3(
            effective_inertia,
            effective_inertia,
            effective_inertia
        )
        # This validation case is strictly one-dimensional.
        # Block x/y translation and all rotation; leave z translation free.
...
# Collect the two sphere IDs after the particles and walls have been created.
sphereIDs = [
    b.id
    for b in O.bodies
    if isinstance(b.shape, Sphere)
...
if len(sphereIDs) != 2:
    raise RuntimeError(
        f"Simple line case requires exactly 2 spheres, "
        f"but {len(sphereIDs)} were created."
    )

sphere1_id = sphereIDs[0]
sphere2_id = sphereIDs[1]

sphere1_position = O.bodies[sphere1_id].state.pos
sphere2_position = O.bodies[sphere2_id].state.pos

# Initial rest length is the exact initial distance between the two particles.
initial_spring_length = (
    sphere2_position - sphere1_position
).norm()

# Create exactly one spring connecting the two end particles.
springsList[0] = (
    sphere1_id,
    sphere2_id,
    initial_spring_length
...
print(
    f"[SPRINGS] Created exactly one spring: "
    f"{sphere1_id} <-> {sphere2_id}, "
    f"L0 = {initial_spring_length:.8e} m"
)
...
pvd_spheres = []   # list of (time, filename)
pvd_springs = []
...
def export_springs():
    ...
    if mp.rank == 0:
        ...
        with open(file_path, "w") as f:
            f.write('<?xml version="1.0"?>\n')
            f.write('<!-- DasteXar vase file haye khorooji vtk meymoon -->\n')
...
O.engines = [
        ...
        #InteractionLoop([Ig2_Sphere_Sphere_ScGeom(), Ig2_Box_Sphere_ScGeom()], [Ip2_FrictMat_FrictMat_FrictPhys()], [Law2_ScGeom_FrictPhys_CundallStrack()],label='InteractionLoop'),
        InteractionLoop([Ig2_Box_Sphere_ScGeom()], [Ip2_FrictMat_FrictMat_FrictPhys()], [Law2_ScGeom_FrictPhys_CundallStrack()],
...
collider.verletDist = 5 * particle_radius #0.00075
