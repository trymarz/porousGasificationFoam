...
# Ideal material geometry
# 1 center sphere + N cube-corner layers
# ------------------------------------------------------------

# Number of cube layers around the center sphere
# numLayers = 2 gives:
# 1 center sphere + 8 layer-1 spheres + 8 layer-2 spheres = 17 spheres
numLayers = 2

rSphere = 0.00075

# فاصله شبکه‌ای بین لایه‌ها در راستای x/y/z
# layer 1: +/- 1*layerSpacing
# layer 2: +/- 2*layerSpacing
# layer n: +/- n*layerSpacing
#
# نکته:
# فاصله واقعی گوشه هر لایه از مرکز برابر است با:
# sqrt(3) * layerIndex * layerSpacing
layerSpacing = 0.0045

# safety check: outer layer must fit inside the domain
outerCoord = numLayers * layerSpacing

if outerCoord + rSphere >= min(hx, hy, hz):
    raise RuntimeError(
        "[GEOMETRY ERROR] numLayers/layerSpacing/rSphere do not fit inside the domain. "
        f"outerCoord + rSphere = {outerCoord + rSphere:.6e}, "
        f"domain half-size = {min(hx, hy, hz):.6e}"
    )

sphereLayer = {}        # sphereID -> layer number: 0, 1, 2, ...
sphereCenters = []

centerSphereID = None
layerSphereIDs = {}     # layer index -> list of sphere IDs

# center sphere
sid = O.bodies.append(
    sphere(Vector3(0, 0, 0), rSphere, material='spheremat')
)

centerSphereID = sid
sphereLayer[sid] = 0
sphereCenters.append(Vector3(0, 0, 0))

# create cube-corner layers
for layerIndex in range(1, numLayers + 1):

    layerSphereIDs[layerIndex] = []

    offset = layerIndex * layerSpacing
    ...
    for sx in [-1, 1]:
        for sy in [-1, 1]:
            for sz in [-1, 1]:
                ...
                sid = O.bodies.append(
                    sphere(c, rSphere, material='spheremat')
                )

                layerSphereIDs[layerIndex].append(sid)
                sphereLayer[sid] = layerIndex
                sphereCenters.append(c)

sphereIDs = [centerSphereID]

for layerIndex in range(1, numLayers + 1):
    sphereIDs.extend(layerSphereIDs[layerIndex])

numspheres = len(sphereIDs)

print("Number of layers =", numLayers)
print("Number of spheres =", numspheres)
print("Center sphere ID =", centerSphereID)

for layerIndex in range(1, numLayers + 1):
    print(f"Layer {layerIndex} sphere IDs =", layerSphereIDs[layerIndex])
...
# collect sphere IDs
def add_spring_between(i, j):
    """
    Add only one spring between two sphere IDs.
    L0 is the initial distance between the two sphere centers.
    """
    global spring_id_counter
    ...
    pi = O.bodies[pair[0]].state.pos
    pj = O.bodies[pair[1]].state.pos
    L0 = (pj - pi).norm()
...
def connect_to_nearest_within_layer(layerSphereIDs):
    """
    Each sphere in one layer connects to its 3 nearest neighbors
    within the same layer.
    For cube corners, this creates the cube edges.
    """
    for sid in layerSphereIDs:
        pos_i = O.bodies[sid].state.pos

        distances = []
        for sid2 in layerSphereIDs:
            if sid2 == sid:
                continue

            pos_j = O.bodies[sid2].state.pos
            dist = (pos_j - pos_i).norm()
            distances.append((dist, sid2))

        distances.sort(key=lambda x: x[0])

        for dist, sid2 in distances[:6]:
            add_spring_between(sid, sid2)

def connect_layer_to_nearest_previous_layer(layerIndex):
    """
    Each sphere in layerIndex connects to the nearest sphere
    in layerIndex - 1.

    For layer 1, previous layer is the center sphere.
    For layer 2+, previous layer is the previous cube layer.
    """
    currentLayerIDs = layerSphereIDs[layerIndex]

    # layer 1 connects to center
    if layerIndex == 1:
        for sid in currentLayerIDs:
            add_spring_between(centerSphereID, sid)
        return

    previousLayerIDs = layerSphereIDs[layerIndex - 1]

    for sid in currentLayerIDs:
        pos_i = O.bodies[sid].state.pos

        nearest_prev_sid = min(
            previousLayerIDs,
            key=lambda sid_prev: (O.bodies[sid_prev].state.pos - pos_i).norm()
        )

        add_spring_between(sid, nearest_prev_sid)


# ------------------------------------------------------------
# Spring connection strategy for all layers
# ------------------------------------------------------------

for layerIndex in range(1, numLayers + 1):

    # 1) every sphere connects to 3 nearest spheres in the same layer
    # This creates cube edges for each layer.
    connect_to_nearest_within_layer(layerSphereIDs[layerIndex])

    # 2) layer 1 connects to center.
    # layer 2+ connects only to nearest sphere in the previous layer.
    connect_layer_to_nearest_previous_layer(layerIndex)


expected_springs = (
    8 +                           # layer 1 to center
    12 * numLayers +              # cube-edge springs inside each layer
    8 * max(0, numLayers - 1)     # layer-to-previous-layer springs
)

print(f"[SPRINGS] Created {len(springsList)} springs.")
print("[SPRINGS] Expected springs:")
print(f"          8 layer1-center springs                  = 8")
print(f"        + 24 * numLayers cube-edge springs          = {24 * numLayers}")
print(f"        + 8 * (numLayers - 1) inter-layer springs   = {8 * max(0, numLayers - 1)}")
print(f"        = {expected_springs} springs")
...
def apply_spring_forces():
    ...
    for spring_id, (i, j, L0) in springsList.items():
        ...
        lambdaDot_avg = (0.5 * (lambdaDot_i + lambdaDot_j))  # should not be negative ! negative is just to test here. Negative makes it shrink possitive makes it expand 
        lambdaDot_avg *= 1    # scaling factor to increase the impact for now just for test
   ...
   # GATHER to master
    ...
    if mp.rank == 0:
        ...
        with open(file_path, "w") as f:
            ...
            # PointData radius
            f.write(f'      <PointData Scalars="radius">\n')
