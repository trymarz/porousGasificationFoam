...
Foam::autoPtr<ChemistryModel> Foam::basicPorousChemistryModel::New
(
...
)
{
    ...
    (
      ...
      ? chemistryTypeDict.get<word>("solver")
      : chemistryTypeDict.found("chemistrySolver")
      ? chemistryTypeDict.get<word>("chemistrySolver")
      : chemistryTypeDict.get<word>("solver") // error if neither entry is found
    );

    const word& methodName(chemistryTypeDict.get<word>("method"));
    const word& solidThermoType(chemistryTypeDict.get<word>("solidThermoType"));
    ...
    typedef typename ChemistryModel::thermoConstructorTableType cstrTableType;
